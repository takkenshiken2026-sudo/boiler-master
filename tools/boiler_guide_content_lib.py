#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2級ボイラー技士 試験ガイド向けの本文・FAQ 生成（量産テンプレ差し替え用）。"""
...
def topic_from_row(row: dict[str, str]) -> str:
    ...
    if len(title) >= 4 and not title.startswith("2級"):
        return title
...
def _heading_よくある誤解(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{topic}では、「ボイラー技士が診断する」「休職勧奨＝解雇」"
        f"「ストレスチェック結果を個別に上司が全部見る」など、現場イメージと制度のズレが誤答の温床になります。"
        f"特に燃焼制御と水処理、安全装置の作動条件の混同は四択で頻出です。",
...
def _heading_教材参考書(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{topic}向けには、{ORG}発行の公式テキスト（乙種第4類）を軸に置くのが基本です。"
        f"問題集は年度・版の表記を確認し、4分野すべてをカバーしているかを目次でチェックしてください。",
        f"教材を増やしすぎるより、1冊を2周してから演習問題・一問一答で穴を見つける方が、"
        f"ボイラー技士の実務イメージもつきやすくなります。{_practice_note(topic)}",
    )
...
def _heading_過去問現在地(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"たとえば「ボイラーの取扱い」と「法令」の問題を取り違えた、など分野名でメモすると復習が楽です。",
...
def _heading_直前期絞り込み(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"数値（使用圧力、安全弁、水位、燃焼制御、検査など）は早見表タブで最終確認すると安心です。",
    )


def _heading_制度改定(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"ボイラー及び圧力容器安全規則・関係法令など、"
        f"{topic}の背景法令は改正やガイドライン更新があります。"
        f"試験は制度の「考え方」とボイラー技士の対応原則が中心ですが、数値要件は公式テキストの版を確認してください。",
...
def _heading_分野位置づけ(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    ...
    return two_paragraphs(
        f"公式テキストの4分野のうち、「{field}」は{topic}の理解に直結する部分です。"
        f"ボイラー技士試験では、現場の判断場面（気づき・相談・配慮・環境改善・専門職連携）として問われます。",
        f"他領域とのつながりを意識すると、事例問題の選択肢が読みやすくなります。"
        f"たとえばボイラーの取扱いの領域は、個別配慮や職場環境評価とセットで出題されることが多いです。",
    )
...
def _heading_基礎知識(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    field = field_name_from_slug(slug) or topic
    return two_paragraphs(
        f"{field}の基礎として、キーワードの定義（ボイラー構造、燃焼制御、水処理、安全装置、関係法令など）を"
        f"用語解説で確認してください。試験では長い定義文の一部が空欄になったり、似た語句と並べ替えられたりします。",
        f"暗記カードを作る場合は「誰が・いつ・何をするか」の3点セットでまとめると、"
        f"事例問題の主体（ボイラー技士／産業医／人事）を見分けやすくなります。{_practice_note(topic)}",
    )
...
def _heading_頻出論点(topic: str, slug: str, _genre: str, _ctx: dict) -> str:
    field = field_name_from_slug(slug) or topic
    return two_paragraphs(
        f"{field}では、ボイラー技士の具体的行動（早期発見、傾聴、記録、専門職へのつなぎ、職場環境へのフィードバック）が"
...
def _heading_受験資格(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{EXAM}（乙種第4類）の受験資格は、{OFFICIAL}の受験要項で確認します。"
        f"ボイラー技士としての位置づけや、実務経験・研修の要件など、自分に該当するかをチェックリスト化してください。",
...
def _heading_申込前チェック(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"公式テキストの版・4分野の学習進捗 ⑥演習問題で弱点分野の把握。"
...
def _heading_過去問年度別(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"解説を読む際は、ボイラー技士の行動として正しいか／法令上の主体が誰か、を必ず確認してください。"
...
def _heading_間違い分類(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"誤答の理由を「知識不足」「用語混同」「読み飛ばし」「主体の取り違え（ボイラー技士 vs 産業医）」"
        f"に分類すると、次の対策が決まります。混同が多い場合は比較表タブを優先してください。",
        f"ノートの1行例：「第○問／ボイラーの取扱い／『記録の開示範囲』を混同」。"
...
def _heading_ボイラー技士役割(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"ボイラー技士は、{topic}の場面で「早期発見・傾聴・安全配慮・記録・専門職連携・職場環境改善」"
        f"のバランスを取る主体として問われます。医学的診断や治療判断は医師の領域であり、ボイラー技士単独では行いません。",
...
def _heading_プライバシー(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"危険物不調に関する情報は個人情報・要配慮個人情報として慎重に扱います。"
        f"ボイラー技士は業務上必要な範囲で情報を共有し、病名の詳細を不必要に広げない対応が求められます。",
...
def _heading_フォローアップ(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"「復職したら終わり」ではなく、再発予防がボイラー技士の役割として問われます。",
...
def _heading_相談窓口(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"ボイラー技士は相談を受けた場合の初動（傾聴、記録、担当者への連携）を理解しておく必要があります。",
...
def _heading_再発防止(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"再発防止には、ハラスメント防止方針の周知、相談窓口の明示、ボイラー技士向け研修、"
...
def _heading_5ステップ(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        ...
        f"ボイラー技士は③④⑤で中心役割を担いますが、医学的判断は主治医・産業医の領域です。"
...
def _heading_試験目的(topic: str, _slug: str, _genre: str, _ctx: dict) -> str:
    return two_paragraphs(
        f"{EXAM}（乙種第4類）は、ボイラー技士が職場で危険物対策を実践するための"
        f"知識・態度を評価する試験です。部下の変化への気づき、安全配慮、ボイラーの取扱い、環境改善が中心テーマです。",
        f"4分野は公式テキストに沿って構成されています。"
...
HEADING_MAP: dict[str, Callable[..., str]] = {
    ...
    "合格後の手続き": lambda t, s, g, c: two_paragraphs(
        ...
        f"{t}に合格した場合も、ボイラー技士としての実務研修や社内規程の更新が別途必要なことがあります。"
    ...
    "ボイラー技士の役割": _heading_ボイラー技士役割,
    "ボイラー技士の関与": _heading_ボイラー技士役割,
    ...
    "試験の目的と乙種第4類の位置づけ": _heading_試験目的,
...
def _keyword_fallback(heading: str, topic: str, slug: str, genre: str) -> str:
    ...
    return two_paragraphs(
        f"「{heading}」では、{EXAM}の{topic}について、ボイラー技士の現場判断と4分野の知識を結びつけて整理します。"
...
def faq_answer_for(question: str, topic: str, slug: str, row: dict[str, str], faq_index: int = 1) -> str:
    ...
    # FAQ 役割分担で類似回答を避ける
    if faq_index == 1:
        if "ボイラー技士" in q and ("診断" in q or "判断" in q):
            text = (
                "ボイラー技士は医学的診断を行いません。不調の気づき、傾聴、安全配慮、"
            ...
            text = (
                ...
                f"{topic}では、条文や指針の主体（事業者・ボイラー技士・産業医）をセットで押さえてください。"
            )
    elif faq_index == 2:
        if "復職" in q or "復帰" in q:
            ...
            text = (
                f"試験では「{topic}」が、ボイラー技士の行動として正しいか／主体を取り違えていないか、"
...
def lead_for(row: dict[str, str], topic: str) -> str:
    ...
    base = (
        f"{EXAM}の{topic}について、ボイラー技士が現場で迷いやすい論点と"
...
def meta_description_for(row: dict[str, str], topic: str) -> str:
    ...
    if genre == "用語ハブ活用法":
        return (
            ...
            f"用語解説・演習・比較表との学習順と、ボイラー技士が押さえる論点を解説します。"
        )[:165]
    if genre == "試験概要":
        return (
    ...
    return (
        f"{EXAM}の{topic}について、ボイラー技士試験で問われる論点と学習の進め方を解説。"
